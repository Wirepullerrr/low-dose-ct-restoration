...
from ct_restoration.models.cnn import ResidualCNN, validate_model_input


def restore_array(
    model: ResidualCNN, degraded: np.ndarray, device: torch.device | str = "cpu"
) -> np.ndarray:
    ...
    validate_model_input(batch, model.config)
...
def torch_restorer(
    model: ResidualCNN, device: torch.device | str = "cpu"
